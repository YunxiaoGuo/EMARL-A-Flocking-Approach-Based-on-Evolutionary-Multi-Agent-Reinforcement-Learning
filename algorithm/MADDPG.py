        ...
        with torch.no_grad():
            ...
            for agent_id in range(self.args.n):
                if agent_id == self.agent_id:
                    action_prob = self.actor_target_network(o_next[agent_id])
                    actions = Categorical(action_prob.squeeze(0)).sample()
                    u_next.append(actions)
                else:
                    # 因为传入的other_agents要比总数少一个，可能中间某个agent是当前agent，不能遍历去选择动作
                    other_prob = other_agents[index].policy.actor_target_network(o_next[agent_id])
                    actions = Categorical(other_prob.squeeze(0)).sample()
                    u_next.append(actions)
                    index += 1
