...
class algorithm:
    ...
    # soft update
    def _soft_update_actor_target_network(self):
        for target_param, param in zip(self.actor_target_network.parameters(), self.actor_network.parameters()):
            target_param.data.copy_((1 - self.args.tau) * target_param.data + self.args.tau * param.data)

    def _soft_update_critic_target_network(self):
    ...
    # update the network
    def train(self, transitions, other_agents,senior_agent_list, junior_agent_list):
        ...
        # the actor loss
        if self.agent_id in junior_agent_list and np.random.rand()<self.args.p_evolve:
            inherit_agent_id  = np.random.choice(senior_agent_list)
            inherit_agent_id = inherit_agent_id if inherit_agent_id<self.agent_id else inherit_agent_id-1
            self.actor_network.load_state_dict(other_agents[inherit_agent_id].policy.actor_network.state_dict())
            self.actor_target_network.load_state_dict(other_agents[inherit_agent_id].policy.actor_target_network.state_dict())
        else:
            ...
            self._soft_update_actor_target_network()
        self.critic_optim.zero_grad()
        critic_loss.backward()
        self.critic_optim.step()
        self._soft_update_critic_target_network()
